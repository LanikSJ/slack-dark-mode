...
type npx
if [[ "$?" != "0" ]]; then
  echo "Please install Node for your OS.  macOS users will also need to install Homebrew from https://brew.sh"
fi
...
if [[ -d $LINUX_SLACK_RESOURCES_DIR ]]; then
  SLACK_RESOURCES_DIR=$LINUX_SLACK_RESOURCES_DIR
fi
...
if [[ "$UPDATE_ONLY" == "true" ]]; then
  echo && echo "Updating Dark Theme Code for Slack... "
fi
...
if [[ "$UPDATE_ONLY" == "false" ]]; then
    ...
    if [[ -f "$HOME/$SLACK_DIRECT_LOCAL_SETTINGS" ]]; then
      sed -i 's/"bootSonic":"once"/"bootSonic":"never"/g' "$HOME/$SLACK_DIRECT_LOCAL_SETTINGS"
    fi

    if [[ -f "$HOME/$SLACK_STORE_LOCAL_SETTINGS" ]]; then
      sudo sed -i 's/"bootSonic":"once"/"bootSonic":"never"/g' "$HOME/$SLACK_STORE_LOCAL_SETTINGS"
    fi
